...
function ts_setupDatabase() {
    ...
    if( ! ts_doesTableExist( $tableName ) ) {
        ...
        $query =
            ...
            "PRIMARY KEY( login_key, download_date ) );";
            ...
            while( $tries < 3 && $success == 0 ) {
                ...
                */
                ...
                $query = "INSERT INTO $tableNamePrefix". "tickets VALUES ( " .
                    "'','$login_key', CURRENT_TIMESTAMP, '$email', '0' );";
...
function ts_downloadAllowed() {
    ...
    if( $numRows == 1 ) {
        ...
        if( !$blocked ) {
            ...
            ts_log( "$login_key permitted to download by $remoteIP" );
            ...
            if( $downloadReady ) {
                ...
                ts_log( "$login_key denied to download by ".
                        "$remoteIP (blocked)" );
                }
            else {
                ts_log( "$login_key denied to download by ".
    ...
    ts_log( "$login_key denied to download by $remoteIP (not found)" );
...
function ts_download() {
    ...
    if( $numRows == 1 ) {
        ...
        $query = "INSERT INTO $tableNamePrefix". "downloads VALUES ( " .
            "'$login_key', CURRENT_TIMESTAMP, '$file_name', ".
            "'$blocked', '$remoteIP' );";
...
function ts_emailOptIn() {
    ...
    if( ts_downloadAllowed() ) {
        ...
        $query = "UPDATE $tableNamePrefix"."tickets ".
            "SET email_opt_in='$in' WHERE login_key= '$login_key';";
        ...
        if( $numRows == 1 ) {
            ...
            if( $email_opt_in == '1' ) {
                echo "[<a href=\"server.php?action=email_opt_in&in=0&".
                    "login_key=$login_key\">Opt Out</a>] of email updates.";
                }
            else {
                echo "[<a href=\"server.php?action=email_opt_in&in=1&".
                    "login_key=$login_key\">Opt In</a>] to email updates.";
                }
            }
        ...
        echo "[<a href=\"server.php?action=show_downloads&".
            "login_key=$login_key\">Return</a>] to your download page.";
...
function ts_changeEmail() {
    ...
    if( ts_downloadAllowed() ) {
        ...
        $query = "UPDATE $tableNamePrefix"."tickets ".
            "SET email='$email' WHERE login_key= '$login_key';";

        $result = ts_queryDatabase( $query );

        ts_log( "Email changed to $email for ticket $login_key" );
...
<?php
    ...
    $query = "SELECT * FROM $tableNamePrefix"."downloads ".
        "WHERE login_key= '$login_key' ORDER BY download_date DESC;";
    $result = ts_queryDatabase( $query );

    $numRows = mysqli_num_rows( $result );

    echo "$numRows downloads for $login_key:";

    echo " [<a href=\"server.php?action=delete_login_key" .
        "&login_key=$login_key\">DELETE this id</a>]";
...
function ts_sendGroupEmail() {
    ...
    $query = "SELECT * FROM $tableNamePrefix"."tickets ".
        ...
        " $orderFilterClause ".
        "ORDER BY creation_date ASC $batchClause;";
...
function ts_sendEmail_q( $inQuery ) {
    ...
    for( $i=0; $i<$numRows; $i++ ) {
        ...
        echo "[$i] Sending email to $email for ticket $login_key... ";
        
        $emailResult = ts_sendEmail_p( $login_key, $name, $email );
...
function ts_sendEmail_p( $inTickeID, $inName, $inEmail ) {
    ...
    $downloadURL = $fullServerURL.
        "?action=show_downloads&login_key=$inTickeID";
...
function ts_sendAllNote() {
    ...
    if( count( $tagParts ) == 3 ) {
        ...
        if( ( $tagParts[0] == "ALL"  || $tagParts[0] == "BULK" ) &&
            ...
            $query = "SELECT * FROM $tableNamePrefix"."tickets ".
                "WHERE blocked = '0' AND email_opt_in = '1' ".
                "ORDER BY creation_date ASC LIMIT $numToSkip, $batchSize;";
            }
        }
...
function ts_addCouponCodes() {
    ...
    $query = "SELECT login_key, email FROM $tableNamePrefix"."tickets ".
    ...
    for( $i=0; $i<$numRows && $j<$numCodes; $i++ ) {
        ...
        echo "Setting coupon <b><tt>$coupon_code</tt></b> ".
            "for ticket [<b><tt>$login_key</tt></b>] ($email)<br>\n";
...
function ts_sendNote_q( $inQuery, $message_subject, $message_text,
    ...
    if( $inUseBulkEmailer ) {
        ...
        for( $i=0; $i<$numRows; $i++ ) {
            ...
            $allCodes[] = ts_mysqli_result( $result, $i, "login_key" );
    ...
    for( $i=0; $i<$numRows; $i++ ) {
        ...
        $custom_message_text =
            preg_replace( '/#DOWNLOAD_CODE#/', $login_key,
                          $custom_message_text );
